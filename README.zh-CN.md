...
一个用完即走的 VS Code 输入翻译浮层。无需配置 API Key。默认会在插件启动时测试已启用的网页翻译服务，并优先使用延迟最低的可用服务：

1. Microsoft（Bing Translator）
2. Google Translate
3. 百度翻译

其余服务按测速结果依次兜底。测速结果有效期为 10 分钟；过期后的下一次翻译继续使用旧顺序，同时在后台刷新测速。你也可以切换到固定顺序模式；服务失败时会严格按照用户配置从左到右继续兜底。界面支持英语和简体中文，并自动跟随 VS Code 的显示语言。
...
本插件模拟各服务商网页端的公开请求，不使用官方商业 API，也不需要用户密钥。自动模式会在启动时向已启用服务发送一次 `hello` 来测量延迟；之后按测速结果依次发送待翻译文本，某个服务成功后便停止继续请求。
